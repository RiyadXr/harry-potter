        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 animate-fade-in-up">
            <div className={`relative p-6 sm:p-8 rounded-lg shadow-2xl w-full max-w-md border-4 ${theme.border} ${theme.secondary} ${theme.text}`}>
                <button
                    onClick={onClose}
                    aria-label="Close"
                    className={`absolute top-2 right-2 p-1 rounded-full text-current opacity-70 hover:opacity-100 hover:bg-black/20 transition-all focus:outline-none focus:ring-2 focus:ring-current`}
                >
